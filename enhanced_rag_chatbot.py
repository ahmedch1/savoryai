...
# LLM Configuration - Using Ollama (free, open-source, no credentials needed)
# Ollama lets you run models locally for free. Download from https://ollama.ai
OLLAMA_API_URL = os.environ.get("OLLAMA_API_URL", "http://localhost:11434/api/generate")
LLM_MODEL = os.environ.get("LLM_MODEL", "mistral")  # Or use "llama2", "neural-chat", etc.

class LocalLLMClient:
    """Client for local LLM inference via Ollama or fallback to simple generation"""
    
    def __init__(self, api_url: str = OLLAMA_API_URL, model: str = LLM_MODEL):
    ...
    def generate(self, prompt: str, params: dict = None) -> dict:
        ...
        try:
            ...
            print(f"⚠️  Ollama error: {e}. Using fallback generation...")
            return self._fallback_generate(prompt)
    
    def _fallback_generate(self, prompt: str) -> dict:
        """Fallback generation when Ollama is unavailable"""
        # Simple rule-based response for when Ollama is not available
    ...
    def _generate_fallback_response(self, prompt: str) -> str:
        """Generate a basic response without external LLM"""
        lower_prompt = prompt.lower()
        
        if "recommend" in lower_prompt or "suggest" in lower_prompt:
            return "Based on the food options provided, I recommend trying the first option as it has the highest match score with your preferences."
        elif "compare" in lower_prompt:
            return "These are two interesting food choices. The first option offers a unique flavor profile while the second is more traditional."
        else:
            return "That's an interesting request. Among the food items found, the top recommendations match your preferences well."

# Initialize the LLM model
model = LocalLLMClient(api_url=OLLAMA_API_URL, model=LLM_MODEL)

def main():
    """Main function for enhanced RAG chatbot system"""
    try:
        print("🤖 Enhanced RAG-Powered Food Recommendation Chatbot")
        print("   Powered by Local LLM (Ollama) & ChromaDB")
        print("=" * 55)
        
        # Check LLM availability
        if model.available:
            print(f"✅ Using Ollama with model: {model.model}")
            print("   (Make sure Ollama is running: ollama serve)")
        else:
            print("⚠️  Ollama not found. Install from https://ollama.ai")
            print("   Will use fallback responses for now.")
        ...
        collection = create_similarity_search_collection(
            ...
            {'description': 'Enhanced RAG chatbot with local LLM integration'}
        )
...
def enhanced_rag_food_chatbot(collection):
    ...
    print("   Powered by Local LLM (Ollama)")
    print("="*70)
...
def show_enhanced_rag_help():
    ...
    print("=" * 45)
    print("🧠 This chatbot uses local LLM (Ollama) to understand your")
    ...
    print("  • 🧠 Local AI analysis provides contextual explanations")
